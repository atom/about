...
class UpdateView {
  ...
  shouldUpdateActionButtonBeEnabled () {
  ...
  executeUpdateAction () {
    if (this.props.state === UpdateManager.State.UpdateAvailableToInstall) {
      ...
      case UpdateManager.State.UpdateAvailableToInstall:
        updateStatus = (
          <div className='about-updates-item app-update-available-to-install' ref='updateAvailableToInstall'>
            ...
            <span className='about-updates-version' ref='updateAvailableVersion'>{this.props.availableVersion}</span>
  ...
  render () {
    return (
      <div className='about-updates group-start' style={{
        // Change back to 'none' when done testing
        display: this.props.updateManager.state === UpdateManager.State.Unsupported ? 'none' : 'initial'
      }}>
        <div className='about-updates-box'>
          ...
          <button className='btn about-update-action-button' disabled={!this.shouldUpdateActionButtonBeEnabled()} onclick={this.executeUpdateAction.bind(this)}>
